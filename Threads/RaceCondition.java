/*
We know that 3 threds working in parallel i.e t1, t2 and main, if we don't write join method, then main thread will not wait for the completion
of thread t1 and t2 (make count = 20,000) and will execute "System.out.println(c.count);" and print count value which is not necessary to be 20,000.

If t1 and t2 implements increament method at the same time then there my arise "Race Condition" (value of count is not as expected), to
prevent the race condition, Synchronization is used(entering one thread at a time incritical section).
